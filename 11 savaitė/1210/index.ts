...
const buyBtn = document.querySelector('.landBtn') as HTMLElement;
let grassArr: any = [];
//resources
let gold: number = 1000;
let wood: number = 100;
let stone: number = 100;
let population: number = 10;
let landQuantity: number = 0;
let randomNumArr: number[] = [];

//interface for one object
interface OneObj {
    type: string,
    quantity: number,
    n: number,
...
for (let i = 0; i < 100; i++) {
    const grass: HTMLElement = document.createElement('div');
...
function appendResources(num1: number, num2: number, num3: number, num4: number): void {
...
function buyHouse(index: number, w: number, s: number) {
    const buyStoneHouse = document.getElementById(`stoneBtn${index}`) as HTMLElement;
    const buyWoodHouse = document.getElementById(`woodBtn${index}`) as HTMLElement;
    const buyHouse = document.getElementById(`houseBtn${index}`) as HTMLElement;
    const textDiv = document.getElementById(`text${index}`) as HTMLElement;
...
let intervalStone:any;
function produceStone(s: number) {
...
let intervalWood:any;
function produceWood(s: number) {
...
let intervalGold:any;
function produceGold(pop: number) {
...
function objectToArray(t: string, q: number, i: number, l: number): void {
    dataArr.push({type: t, quantity: q, n: i, level: l});
    console.log(dataArr)
}

function upgradeLevel(index: number) {
    let level = 1;
    ...
    levelDiv.onclick = (e) => {
        ...
        for (let i = 0; i < dataArr.length; i++) {
            ...
            if (String(dataArr[i].n) === (e.target as HTMLElement).id && dataArr[i].type === 'wood') {
            ...
            if (String(dataArr[i].n) === (e.target as HTMLElement).id && dataArr[i].type === 'stone') {
            ...
            if (String(dataArr[i].n) === (e.target as HTMLElement).id && dataArr[i].type === 'simple') {
