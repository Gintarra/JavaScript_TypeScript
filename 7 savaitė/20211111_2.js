...
const yearsSelect = document.getElementById("byYears");
const btnYears = document.getElementById("yearsBtn");

smallBox.innerHTML = `<p>Select a film's category from the list.</p>
<select id="mySelect">
    <option value="allGenres">All</option>
    <option value="Dramos">Drama</option>
    <option value="Muzikiniai">Muzikiniai</option>
    <option value="Šeimai">Šeimai</option>
    <option value="Animaciniai">Animaciniai</option>
     <option value="Siaubo">Siaubo</option>
</select>`;
...
function filmPrint(i) {
    box.innerHTML += `
    <div class="card pad d-flex margin">
        <img class="margin_r" src = ${movies[i].image} alt="">
        <div>
            <h1>${movies[i].titleLtu}</h1>
            <h3><i class="fab fa-imdb fa-2x"></i> ${movies[i].imdbRating}</h3>
            <h3>Year: ${movies[i].year}</h3>
            <h3>Genres: ${movies[i].genre}</h3>
            <h4>Description: ${movies[i].description}</h4>
         </div>
    </div>`
}
...
function films(genreSelect, rate) {
    box.innerHTML = "";
    for (let i = 0; i < movies.length; i++) {
        if (movies[i].genre.includes(genreSelect) && movies[i].imdbRating >= rate) {
            filmPrint(i);
        }
        if (genreSelect === "allGenres" && movies[i].imdbRating >= rate) {
            filmPrint(i);
        }
    }
}

//pirma karta visus parodo
//ir jei nekeiciam kategorijos, tik pagal balus filtruoja

allFilms();
//jei nepakeiciu kategorijos, vis tiek noriu. kad filtruotu pagal reitinga
if (select.value === "allGenres") {
    range.oninput = () => {
        let rating = 0;
        rating = range.value;
        films("allGenres", rating);
    }
}
//keiciu kategorija, tada reitinga, ir abiejose veikia funkcijos, kad bent viena oakeitus
//atvaizdavimas pasikeistu
select.onchange = () => {
...
//isfiltruoja pagal metus nurodytus
btnYears.onclick = () => {
    const flt = movies.filter(x => x.year === Number(yearsSelect.value));
     filmPrintArray(flt)
    console.log(flt)
    console.log(yearsSelect.value)
}

//isfiltruoja pagal imdb
function filteredByImdb(rate) {
    const flt = movies.filter(x => x.imdbRating === rate);
    filmPrintArray(flt)
    console.log(flt)
}
...
//isfiltruoja pagal ilgi - ilgesnius uz ivesta skaiciu
function filteredByLength(l) {
    const flt = movies.filter(x => x.description.length > l);
    filmPrintArray(flt)
    console.log(flt)
}

filteredByLength(200)

//isfiltruoja filmus pagal ivest zanra
function filteredByGenres(arg) {
    const flt = movies.filter(x => x.genre.includes(arg));
    filmPrintArray(flt)
    console.log(flt)
}

filteredByGenres("Dramos");

//spausdina visa paduodama array (masyva)
function filmPrintArray(arr) {
    box.innerHTML = "";
    arr.map(x => {
        box.innerHTML += `
    <div class="card pad d-flex margin">
        <img class="margin_r" src = ${x.image} alt="">
        <div>
            <h1>${x.titleLtu}</h1>
            <h3><i class="fab fa-imdb fa-2x"></i> ${x.imdbRating}</h3>
            <h3>Year: ${x.year}</h3>
            <h3>Genres: ${x.genre}</h3>
